...
class CompileShader_sge:
    def main(args):
        ...
        sge_root = os.path.abspath(cur_dir + "/../../..")
        project_root = os.path.dirname(args[CmdLineArg.Project_root.value])

        is_sge_root_exist = os.path.exists(sge_root + "/build") and os.path.exists(sge_root + "/CMakeLists.txt") # TODO: need proper check
        if not (is_sge_root_exist):
            raise Exception("in correct sge_root")
            return
        ...
        for idx, path in enumerate(shaders_path):
            ...
            res = subprocess.run(
                [sge_make, mk_sge_root, mk_project_root, shader_file_path, ], 
...
CompileShader_sge.main(sys.argv)
